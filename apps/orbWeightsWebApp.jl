...
# ╔═╡ c9677b35-66e7-4f03-a4a8-844a08fa9453
begin
    ...
    if !(@isdefined getReactionForm) # To avoid warnings, if cell is re-loaded
        include(folderpath_OWCF*"misc/availReacts.jl") # To process fusion reactions
    end
    if !(@isdefined OWCF_grid) # To avoid warnings, if cell is re-loaded
        include(folderpath_OWCF*"extra/dependencies.jl") # To be able to use functions
    end
    if !(@isdefined ViewingCone) # To avoid warnings, if cell is re-loaded
        include(folderpath_OWCF*"vcone.jl") # To be able to load diagnostic viewing cones
    end
    println()
end

# ╔═╡ 60c3df40-aaa0-11f1-2503-7319872c8794
md"""
# orbWeightsWebApp
...
- orbWeights\_2Dto4D.jl (if not automatically run as a part of calcOrbWeights.jl)
...
- filepath\_WF - The path to the WF-signal to be visualized as well. Should be an output file of the OWCF/ps2WF.jl script. Enable with plot_WF - String
- FI\_species - The fast-ion particle species. E.g. "D", "T" etc. Please see the OWCF/misc/species_func.jl file for a list of available particle species options - String
...
begin
    ...
    diagnostic_name = "KM14" # Diagnostic sightline aestethic keyword. E.g: "TOFOR", "AB" or ""
    filepath_W = folderpath_OWCF*"apps/example_data/orbWeights4D_JET_99971L72_at8,9s_KM14_T-D--n-4He---GS_159x33005.jld2"
    (plot_Fos = true) && (filepath_Fos3D = folderpath_OWCF*"apps/example_data/F_os_3D_JET_99971L72_at48,9s_D_6x101x102.jld2") # Enable and specify an orbit-space fast-ion distribution file (3D-format)
    (plot_S = true) && (filepath_S = folderpath_OWCF*"apps/example_data/spec_JET_99971L72_at8,9s_KM14_T-D--n-4He.jld2") # Enable and specify a signal file to include in the plotting
    ...
    (plot_WF = true) && (filepath_WF = folderpath_OWCF*"apps/example_data/ps2WF_results_JET_99971L72_at8,9s_AB_T-D--n-4He_126x101x104.jld2")
    FI_species_init = "D" # Specify with D, T, p, 3he etc
    xlabel = "Neutron energy [keV]" # Example neutron energy: "Neutron energy [keV]". Example projected velocity: "Projected velocity [m/s]"
    ylabel = "Neutron count [(keV*s)^-1]" # Example neutron count: "Neutron count [(keV*s)^-1]". Example projected velocity signal: "Projected velocity signal [m^-1]"
...
begin
    ...
    #diagnostic_filepath = fW_array[end-2]
    ...
    elseif (@isdefined reaction)
        ...
        FI_species = FI_species_init # Already defined
    end
    ...
    verbose && println("Defining final variables needed for web application... ")
...
function plot_inputs()
	...
	return PlutoUI.combine() do Child
		...
		inputs=[
			...
			md""" Show f(E,pm,Rm): $(
				Child("show_f", Switch(default=false))
			)""",
			md""" f-index: $(
				Child("irec", Slider(Rec_array))
			)""",
			md""" Colorbar scale: $(
				Child("cbarscale", Select(["zero2one" => "0.0-1.0", "zero2aTenth" => "0.0-0.1"]))
			)""",
			md""" Coordinate space: $(
			...
			md""" Show delta function signal: $(
				Child("show_delta_signal", Switch(default=false))
			)""",
			md""" Measurement bin, Ed (): $(
...
let
	...
	include_Fos = my_plot_inputs.show_f
	irec = my_plot_inputs.irec
	colorbar_scale = my_plot_inputs.cbarscale
	phase_space = my_plot_inputs.coordspace
	show_coordinate = my_plot_inputs.showcoord
	show_delta_signal = my_plot_inputs.show_delta_signal
	...
	EPRc = EPRCoordinate(M, E, pm, Rm; amu=getSpeciesAmu(FI_species), q=getSpeciesEcu(FI_species))
    o = get_orbit(M,EPRc; wall=wall, toa=true, limit_phi=true)
    ...
    if include_Fos && plot_Fos # If you want to plot it (include_Fos) and it's been loaded (plot_Fos)
        plt_Fos = Plots.heatmap(Rm_array, pm_array, (F_os_4D[irec,Ei,:,:])./maximum(F_os_4D[irec,Ei,:,:]), colorbar=true, title="Fast-ion distribution slice  ($(round(maximum(F_os_4D[irec,Ei,:,:]), sigdigits=4)) = 1.0)", clims=clims, fillcolor=cgrad([:white, :darkblue, :green, :yellow, :orange, :red]), titlefontsize=10)
    ...
    if include_Fos && plot_Fos
        ...
        plt_WFdens = Plots.heatmap(Rm_array, pm_array, (WF_dens)./maximum(WF_dens), colorbar=true, title="WF-density slice  ($(round(maximum(WF_dens), sigdigits=4)) = 1.0)", clims=clims, fillcolor=cgrad([:white, :darkblue, :green, :yellow, :orange, :red]), titlefontsize=10)
	...
	if include_Fos && plot_Fos # Three rows with two plots each (total 6)
		myplt = Plots.plot(plt_sig, plt_weights, 
						       plt_Fos, plt_WFdens, 
						  	 plt_top, plt_crs,
		...
		myplt = Plots.plot(plt_sig, plt_weights,
						  ...
						  , right_margin=6Plots.mm, left_margin=6Plots.mm)
    end
	Plots.plot(myplt)
end
