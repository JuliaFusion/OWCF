...
@everywhere begin
    ...
    filepath_OG = folderpath_OWCF*"apps/example_data/orbGrid_JET_99971L71_at48,9s_D_6x101x102.jld2" # You can choose to load the orbit-space grid points from a pre-computed orbit-grid .jld2-file...
    ...
    includeExtractTopoBounds = true # If true, then a .jld2 file with the boundaries of the topological map will be extracted and saved as well
    keyname_diagHDF5 = "" # Needed to extract info from .hdf5-file, if weightsFileJLD2 is set to false. 
    saveTransitTimeMaps = true # If true, then poloidal and toroidal transit times for all valid orbits will be included in the results file (see description above)
    timepoint = "48,900" # "XX,YYY". If unknown, just leave as nothing. The algorithm will try to figure it out automatically.
    tokamak = "JET" # If unknown, just set ""
    TRANSP_id = "99971L72" # If unknown, just set ""
