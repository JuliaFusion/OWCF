...
@everywhere begin
    ...
    analytic = false
    debug = false
    diagnostic_filepath = folderpath_OWCF*"vc_data/TOFOR/TOFOR.vc" # The filepath to the LINE21 (or OWCF/extra/createCustomLOS.jl output) file containing viewing cone data. Or, automatic sightlines include "TOFOR", "AB" and "".
    diagnostic_name = "KM14" # For aesthetic purposes
    instrumental_response_filepath = folderpath_OWCF*"vc_data/TOFOR/diamond_scintillator_response_matrix_proxy.jld2" # Should be the filepaths to three .txt-files or one .jld2-file. Otherwise, leave as ""
    instrumental_response_output_units = "keV^-1_s^-1" # Should be specified as described in OWCF/misc/convert_units.jl. If instrumental_response_filepath=="", leave as ""
    Ed_min = 10000.0 # keV (or m/s if 'reaction' input variable is specified on form (3) (please see OWCF/misc/availReacts.jl for further explanation))
    Ed_max = 18000.0 # keV (or m/s if 'reaction' input variable is specified on form (3) (please see OWCF/misc/availReacts.jl for further explanation))
    Ed_diff = 50.0 # keV (or m/s if 'reaction' input variable is specified on form (3) (please see OWCF/misc/availReacts.jl for further explanation))
    ...
    npm = 0
    nRm = 0
    og_filepath = folderpath_OWCF*"apps/example_data/orbGrid_JET_99971L71_at48,9s_D_6x101x102.jld2" # "", by default. Specify a string with the path to a .jld2 file computed with the calcOrbGrid.jl to use that orbit grid
    plot_results = true # Set to true, if 6 (pm,Rm) slices spaced equally in the (Emin,Emax) range (or extrema(E_array) range) should be plotted, to visualize the results
