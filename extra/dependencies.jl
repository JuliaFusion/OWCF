...
"""
    backward_difference_matrix(space_size::Tuple, dim::Int64)
    backward_difference_matrix(-||-; ignore_grid_resolution=false, boundary_condition=:forward)

Compute the backward difference matrix (l1) for a coordinate space of size 'space_size' along dimension 'dim'. 
For example, if space_size=(10,30) and dim=2, then 'backward_difference_matrix(space_size,dim)' will output 
...
grid that have ONE INDEX SMALLER (backward difference) in the 2nd dimension (since dim=2 for this example) of the (10,30) 
...
The keyword arguments are:
    ...
    - boundary_condition - Since the backward difference algorithm requires the value at the grid point with ONE INDEX SMALLER
                           than the grid point of interest, the value of the finite difference at the grid point with index i=1 
                           needs be computed via a boundary condition. Available options for the boundary condition include :forward (forward difference) 
                           and :neumann (the finite difference is assumed to be equal to the function value at the boundary). 
                           By default, use the :forward difference to compute the finite difference for the grid point at the boundary - Symbol
"""
function backward_difference_matrix(space_size::Tuple, dim::Int64; ignore_grid_resolution::Bool = false, boundary_condition = :forward)
    ...
    if boundary_condition == :forward
        h_inv_i_boundary = - h_inv # Forward difference algorithm. Index 'i' is 1
        h_inv_j_boundary = h_inv # Forward difference algorithm. Index 'j' is next to the boundary (j = 2 in the dim:th dimension)
    elseif boundary_condition == :neumann
        ...
        error("Only :forward and :neumann are currently supported for the keyword argument input 'boundary_condition'. Got a value of $(boundary_condition)")
    end
    ...
    for (i,space_coord_i) in enumerate(space_indices) # For every space point (every row in the finite difference matrix)
        if space_coord_i[dim]==1 # Boundary point in the dim:th dimension
            space_coord_j = [sc for sc in Tuple(space_coord_i)] # Convert to Vector, to make space_coord_j mutable
            space_coord_j[dim] = 2 # The Cartesian index of the point next to the boundary in the dim:th dimension
        ...
        for (j,space_coord_j) in enumerate(space_indices) # -||- (for every column in the finite difference matrix)
            if sum(Tuple(space_coord_i) .- Tuple(space_coord_j))==1 && (space_coord_i[dim]-space_coord_j[dim])==1 # If the coordinates differ by one (neighbours with j coordinate larger than i coordinate) and the difference is in the right dimension (dim)
...
"""
    backward_difference_matrix(space_size::Tuple)
    backward_difference_matrix(-||-; ignore_grid_resolution=false, verbose=false)

Compute the backward difference matrix (L1) for a coordinate space of size 'space_size'. The size of
...
Please see function 'backward_difference_matrix(space_size,dim)' above for detailed info.
The keyword arguments are:
    - ignore_grid_resolution - If set to true, the grid resolution will be ignored and elements in the output matrix
                               will have values only in the set {0, -1, 1} - Bool
    - verbose - If set to true, the function will be talkative! - Bool
"""
function backward_difference_matrix(space_size::Tuple; ignore_grid_resolution::Bool = false, verbose::Bool=false)
    ...
    for idim=1:length(space_size) # For each dimension
        verbose && println("Computing backward difference matrix for dimension $(idim) of $(length(space_size))... ")
        L1[idim] = backward_difference_matrix(space_size, idim; ignore_grid_resolution=ignore_grid_resolution) # The backward difference matrix for this particular dimension
    end
