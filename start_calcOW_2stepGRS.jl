...
@everywhere begin
    ...
    debug = false
    diagnostic_filepath = "vc_data/TOFOR/TOFOR.vc" # The filepath to the LINE21 file containing viewing cone data. Or, automatic sightlines include "TOFOR", "AB" and "". If diagnostic_angles wish to be used, set as "" or nothing
    ...
    Ed_min = 4.3*10^3 # keV (or m/s if analyticalOWs===true)
    Ed_max = 4.6*10^3 # keV (or m/s if analyticalOWs===true)
    Ed_diff = 5 # keV (or m/s if analyticalOWs===true)
    E_array = [400.,900.,1900.,2500.,3500.,4000.] # keV. Array can be specified manually. Otherwise, leave as 'nothing'
    Emin = 0.0 # keV
    Emax = 000.0 # keV
    ...
    folderpath_o = "OUTPUT/" # Output folder path. Finish with '/'
    ...
    thermal_temp_axis = 5. # keV. Please specify this if filepath_thermal_distr and filepath_FI_cdf are not specified
    thermal_dens_axis = 1.0e18 # m^-3. Please specify this if filepath_thermal_distr and filepath_FI_cdf are not specified
