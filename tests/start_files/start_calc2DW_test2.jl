...
@everywhere begin
    analytic = true
    debug = false
    diagnostic_filepath = folderpath_OWCF*"vc_data$(folder_delimiter)AB$(folder_delimiter)AB.vc" # Currently supported: "TOFOR", "AB" and ""
    diagnostic_name = "AB"
    ...
    Ed_min = 4300.0 # keV (or m/s if 'reaction' input variable is specified on form (3) (please see OWCF/misc/availReacts.jl for explanation))
    Ed_max = 4600.0 # keV (or m/s if 'reaction' input variable is specified on form (3) (please see OWCF/misc/availReacts.jl for explanation))
    ...
    filepath_equil = folderpath_OWCF*"equilibrium$(folder_delimiter)JET$(folder_delimiter)g96100$(folder_delimiter)g96100_0-53.0012.eqdsk" # for example "equilibrium/JET/g96100/g96100_0-53.0012.eqdsk" or "myOwnSolovev.jld2"
    ...
    thermal_dens_axis = 0.5e19 # m^-3. Please specify this if filepath_thermal_distr and filepath_FI_cdf are not specified
    thermal_dens_profile_type = :FLAT # Currently available options are :DEFAULT and :FLAT
