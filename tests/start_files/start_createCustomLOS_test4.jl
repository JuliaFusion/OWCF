...
#         in degrees between the LOS vector and the R-axis in the (R,z) plane, at phi. The phi angle 
#         is the standard cylindrical coordinate angle. The LOS vector is assumed to run along the LOS, 
#         TOWARDS the detector location.
...
@everywhere begin
    ...
    CTS_like = false
    debug = false
    detector_location = [6.0, -40.0, -1.0] # TOFOR (proxy): [2.97, 0.0, 18.8] /// NE213 (proxy): [8.35,2.1,-0.27]. When coordinate_system==:magrel, detector_location can be set to whatever (the script will figure it out automatically from the angle and LOS_length)
    ...
    LOS_name = "createCustomLOS_test4"
