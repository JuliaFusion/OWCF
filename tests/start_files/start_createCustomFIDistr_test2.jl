...
# Input variables
@everywhere begin
    constant_Rz = true # By default, assume that the fast-ion distribution is not constant in (R,z)
    distribution_type = :collisional # The type of the fast-ion distribution to be created. Currently supported options are :gaussian, :collisional and :custom
    filename_out = "createCustomFIDistr_test2"
    folderpath_out = folderpath_OWCF*"tests$(folder_delimiter)outputs$(folder_delimiter)" # The output folder in which to save the custom fast-ion distribution data
    max_E = 150.0 # keV
    max_p = 1.0 # Dimensionless
    max_R = 4.0 # Meters
    max_z = 2.0 # Meters
    min_E = 5.0 # keV
    min_p = -1.0 # Dimensionless
    min_R = 2.0 # Meters
    min_z = -2.0 # Meters
    nE = 30 # Number of energy grid points
    np = 20 # Number of pitch grid points
    nR = 20 # Number of major radius grid points
    nz = 22 # Number of vertical grid points
    save_plots = $plot_test_results
    tot_N_FI = 1.0e19 # Total number of fast-ions, i.e. ∫ f(E,p,R,z) 2*pi*R dEdpdRdz
    verbose = false
    ...
    elseif distribution_type==:collisional
        assume_Ti_equalTo_Te = true # By default, do not assume that Ti==Te
        ...
        filepath_equil = folderpath_OWCF*"equilibrium$(folder_delimiter)JET$(folder_delimiter)g96100$(folder_delimiter)g96100_0-53.0012.eqdsk" # E.g. "g94701_0-50.7932.eqdsk" or "solovev_equilibrium_2024-09-30.jld2"
        filepath_thermal_profiles = "" # E.g. "96100J01.cdf", "my_custom_thermal_profiles.jld2" or ""
        inj_E = 110.0 # keV
        inj_p = 0.65 # Dimensionless
        inj_R = 3.5 # Meters
        inj_z = 0.3 # Meters
        ...
        thermal_species = "T" # Please see OWCF/misc/species_func.jl for a list of available particle species
        ...
        if dampen # If you specify the 'dampen' input variable to true, please specify...
            ...
            E_tail_length = 80 # The length of the energy tail below the damping energy. In keV
        end
        ...
        if filepath_thermal_profiles=="" # If the 'filepath_thermal_profiles' input variable is left unspecified please specify... 
            dens_on_axis = 1.0e20 # Thermal ion density on-axis. In m^-3
            temp_on_axis = 5.0 # Thermal ion temperature on-axis. In keV
        end
    else # must be :custom
        if constant_Rz
...
include(folderpath_OWCF*"extra$(folder_delimiter)createCustomFIDistr.jl")